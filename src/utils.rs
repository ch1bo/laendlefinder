...
pub fn load_properties_from_csv(path: &str) -> Result<Vec<Property>> {
    ...
    if !path.exists() {
        println!("CSV file {} does not exist, creating a new one", path.display());
        return Ok(Vec::new());
    }
    ...
    for result in reader.deserialize() {
        let property: Property = result
            .with_context(|| "Failed to deserialize property from CSV")?;
        properties.push(property);
    }
    
    println!("Loaded {} properties from {}", properties.len(), path.display());
